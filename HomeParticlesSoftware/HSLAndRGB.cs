...
namespace HomeParticlesSoftware
{
    public class HSLAndRGB
    {
        ...
        public HSLAndRGB(float H, float S, float L)
        {
            Hue = H;
            Saturation = S;
            Luminosity = L;
        ...
        public static HSLAndRGB FromRGB(Color Clr)
        {
            return FromRGB(Clr.R, Clr.G, Clr.B);
        }

        public static HSLAndRGB FromRGB(Byte R, Byte G, Byte B)
        {
            float _R = (R / 255f);
            float _G = (G / 255f);
            float _B = (B / 255f);
